...
def encode(s):
    return [stoi[c] for c in s if c in stoi] # encoder: take a string, output a list of integers
def decode(l):
...
openweb_dataset = load_dataset("openwebtext", num_proc=num_proc_load_dataset, trust_remote_code=True)['train']
# take 500k samples from openweb dataset by random
openweb_dataset = openweb_dataset.shuffle(seed=2357).select(range(500000))
